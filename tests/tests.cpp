...
//             while (!__exit_state.gotAnswer()) { audio_i.deviceUpdateAll(); audio_o.deviceUpdateAll(); }
    ...
    SVKFW_TEST_BEG(Test6, "JSON Parsing")
        SVKFW_SUBTEST_BEG(st1, "Save/Load from string") {
            ...
            Tests::testAssert(json_example == res_json, "writeToString result doesn't match reference");
        } SVKFW_SUBTEST_END(st1);
    SVKFW_TEST_END(Test6);
}; // Simple END
