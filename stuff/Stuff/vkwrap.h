...
/*  ================================================================================================
    ...
    Structure (for now) of Vulkan objects wrapper.

    For Vulkan objects - anything that has handles:
        - destroyable: anything that has a "Vk*CreateInfo" structure and/or "vkCreate*"/"vkDestroy*" functions;
        - non-destroyable: like Physical Devices/Queue Families - a resource to work with that must be selected via Vulkan functions;

    TODO:
    Also InfoHandlers are planned for every Vulkan struct that has sType. This also includes VkObjectNameCreateInfo structs,
    which are sometimes used independently.
    The biggest code generation issue is to find a "elementCount"/"elementSize"/etc. member for a "pElements" member
    which sets the length of an array passed through struct. Sometimes the names don't match.


    // VKFW Object namespace template:

    namespace VKFWObjectName {
    // Classes declaration

        // Handlers are now generated in the file "vkwrap_handlers.h"
        struct    BaseHandler;
        struct        Handler; - destroys vulkan object in destructor
        struct BaseVecHandler; - if there is any use of multiple VkObjectName (i.e. Vulkan structures that take an array of VkElementName)
        struct     VecHandler; - destroys all vulkan objects in destructor

        struct Builder;        - if there is a "VkObjectNameCreateInfo" struct
        struct Manager;        - if a resource is not created by Vulkan and must be selected (i.e. VKFWPhysDevice/VKFWQueueFamily)


    // Functions declaration

        // VKFWObject-related utility functions declaration


    // Classes definition

        // TODO: add assignment operators, which are already generated.

        struct BaseHandler {
        // Data:

            VkObjectName object_handle;                // Underlying Vulkan object handle - it is always the first member of the struct
            VkOtherObjectName another_object_handle;   // Multiple additional members supported, but on practice
                                                       // it is usually VkDevice (or VkInstance - once).
                                                       // Another Vulkan object's handle used in vkDestroyObjectName()
            const VkAllocationCallbacks *destroy_callback = nullptr; // The last parameter of vkDestroyObjectName()


        // Constructors:

            // Constructor - default
            // Constructor - from values (includes parameter of VkObjectName type)
            // Constructor - copy BaseHandler
            // Constructor - move BaseHandler (uses argument's clearHandler() method)

            //  Destructor - calls clearHandler()


        // Pointer access:

            // const VkObjectName* ptr() const { return &object_handle; }


        // Destroy Vk object:

            // Only if such function exists
            void destroyVkObject() { vkDestroyElementName(another_object_handle, object_handle, destroy_callback); }


        // Clear state:

            void clearHandler() {} // Clear Data. Useful when using Handler, to prevent destroying Vk object after it was moved
                                   // (or copied - then probably should be called manually in the Handler the object was copied from)
        }; // BaseHandler END


        // If vkCreateObjectName/vkDestroyObjectName functions exist, otherwise: using Handler = BaseHandler;
        struct Handler : BaseHandler {
        // Constructors:

            // Constructor - default
            // Constructor - from values (includes parameter of VkObjectName type)
            // Constructor - copy BaseHandler (this object's destruction invalidates BaseHandler)
            // Constructor - move BaseHandler
            // Constructor - copy Handler (either this or copied object's destruction invalidates the other one)
            // Constructor - move Handler (uses argument's clearHandler() method, so this handler won't be invalidated until its own destruction)

            //  Destructor - calls vkDestroyObjectName()
        }; // Handler END
        ...
        // Data:
            ...
            std::vector<VkObjectName> object_handles;                  // Vector of underlying Vulkan object handles - it is always the first member of the struct
            VkOtherObjectName other_object_handle;                     // VkInstance, VkDevice etc. - another Vulkan object's handle used in vkDestroyElementName()
            const VkAllocationCallbacks *destroy_callback = nullptr;   // The last parameter of vkDestroyObjectName() for each VkObjectName in "object_handles"


        // Constructors:

            // Constructor - default
            // Constructor - from values (includes parameter of "const std::vector<VkObjectName> &" type)
            // Constructor - copy BaseHandler
            // Constructor - move BaseHandler (uses argument's clearHandler() method)
            // Constructor - copy BaseVecHandler
            // Constructor - move BaseVecHandler (uses argument's clearVecHandler() method)

            //  Destructor - calls clearVecHandler()


        // Access by index:

            VkObjectName operator[](uint32_t _index) { return object_handles[_index]; }
            BaseHandler getBaseHandler(uint32_t _index) { return { object_handles[_index], other_object_handle }; } // Uses BaseHandler Constructor - from values


        // Pointer and count: - for Vulkan struct's array members

            const VkObjectName* ptr() const { return object_handles.data(); }
            uint32_t count() const { return object_handles.size(); }


        // Destroy all Vk objects:

            // If such function exists
            destroyAllVkObjects() {
                for (auto obj_handle : object_handles)
                    vkDestroyObjectName(other_object_handle, obj_handle, destroy_callback);
            }

            // TODO: also Vulkan may have function to destroy array of "object_handles"
            //       (it has vkCreate* for an array of VkSwapchainKHR), check those cases
        ...
        // Clear state:
            ...
            void clearVecHandler() {
                ...
                // All other_object_handle default assignments
                    // Note: multiple other_object_handle members supported, although usually
                    // there is zero or one - VkDevice; also VkInstance - once, for VkDevice.
            }

        }; // BaseVecHandler END


        // If vkCreateObjectName/vkDestroyObjectName functions exist, otherwise: using VecHandler = BaseVecHandler;
        struct VecHandler {
        // Constructors:

            // Constructor - default
            // Constructor - from values (includes parameter of "const std::vector<VkObjectName> &")
            // Constructor - copy BaseHandler    (this object's destruction invalidates BaseHandler)
            // Constructor - move BaseHandler    (uses argument's clearHandler() method)
            // Constructor - copy BaseVecHandler (this object's destruction invalidates BaseVecHandler)
            // Constructor - move BaseVecHandler (uses argument's clearVecHandler() method)
            // Constructor - copy VecHandler     (either this or copied object's destruction invalidates the other one)
            // Constructor - move VecHandler     (uses argument's clearVecHandler() method, so it won't be invalidated until its own destruction)

            //  Destructor - calls (if exists) vkDestroyObjectName() for each element of "object_handles"
        }; // VecHandler END



    // If a Vulkan object has vkCreateObjectName(), it can be created using this struct as a wrapper.
    // Usage: simpler VkObjectNameCreateInfo initialization + vkCreateObjectName() call + BaseHandler/Handler init

        struct Builder {
        // Data: - does not contain VkObjectNameCreateInfo, only members other than sType, pNext and flags (the last two are set via make/makeHandler/initHandler/etc. methods)

            // members specific to VkObjectCreateInfo struct, has other_object_handles
            // used in vkCreateObjectName(other_object_handle, object_handle, create_callback)


        // Constructors:

            Constructor - default constructor, has parameters: "VKFWOtherObjectName::BaseHandler = {}" for each other_object_handle used in vkDestroyObjectName()
            Constructor - copy (usually very heavy, not recommended to use)
            Destructor  - calls clearBuilder()


        // Create handlers/CreateInfo struct: - have pNext, flags and create/destroy allocation callbacks as arguments

            // void initHandler() {}                      // calls make(), initializes Handler object in place -- NOT NEEDED ANYMORE, now there's BaseHandler + copy assignment operator
            BaseHandler makeBaseHandler() {}           // calls make()
            Handler makeHandler() {}                   // calls make()
            VkObjectName make() {}                     // calls makeInfoStruct()
            VkObjectNameCreateInfo makeInfoStruct() {} // does 90% of the work


        // Clear state: - useful when one Builder object is used to create multiple Vulkan objects

            void clearBuilder() {}


        // Setters: - set VkObjectNameCreateInfo struct members in a more direct way

            void set*(..) {}


        // Additional methods: - everything else is here
                //// like adding elements to arrays, for example, VKFWInstance::Builder().addExtensions(..)
        }; // Builder END



    // If a Vulkan object is a resource that isn't created (chosen, like Physical device):

    // There is a Manager struct with testPickOne/testPickAll/rankPickOne/rankPickAll methods, taking user-defined test/rank function,
    // which can be used to choose this resource.

    // InfoElement struct exists to be passed as a parameter to test/rank functions.
    // It contains references to all features and properties of VkObjectName [i]


        struct InfoElement {
        // Data: reference to a property from Manager's vector of properties


        // Constructors:

            // Constructor - data by reference


        // Additional methods:

            // Which can simplify member access / info gathering from data
        }; // InfoElement END



        struct Manager {
        // Data: vector of properties/other info on managed entities, requires loading (loadInfo)


        // Constructors:

            Constructor - default, if has other vulkan objects as dependencies,
                          they can be set via this constructor (defaulted to VK_NULL_HANDLE)
        ...
        // Access element info by index:
            ...
            const InfoElement getElemInfo(uint32_t _index) {}
        ...
        // Pick methods: - pick One or All entities that satisfy user requirements (expressed by test/rank functions)

            void testPickOne(const TestFunc &_func, ...) {}
            void testPickAll(const TestFunc &_func, ...) {}
            void rankPickOne(const RankFunc &_func, ...) {}
            void rankPickAll(const RankFunc &_func, ...) {}
        }; // Manager END
...
//  ============  Utilities  ============  \\
    ...
    // Try to find a better name or better: don't use at all
    using VoidFuncVoid = void (*)(void);
...
// used to simplify work with all the "const char **" parameters in Vulkan
    ...
    enum class VKFWCompareOp : uint8_t {
        NE, // not equal
        EQ, // equal
        LT, // less than
...
//  ============  Vulkan (general)  ============  \\
    ...
    namespace VulkanAPIVersion {
        ...
        inline UVec3 divide(uint32_t _version, uint32_t *_variant = nullptr) {
            if (_variant != nullptr)
                *_variant = VK_API_VERSION_VARIANT(_version);
...
//  ============  Vulkan Instance  ============  \\
    ...
    // Classes definition
        ...
        // Create handlers/CreateInfo struct:
            ...
            void initHandler(BaseHandler &_handle, void *_p_next = nullptr, VkInstanceCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                _handle.instance = make(_p_next, _flags, _create_cb);
                _handle.destroy_callback = _destroy_cb;
            }

            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkInstanceCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkInstanceCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), _destroy_cb };
            }

            VkInstance make(void *_p_next = nullptr, VkInstanceCreateFlags _flags = 0u,
                            const VkAllocationCallbacks * _create_cb = nullptr) {
                VkInstanceCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkInstance _instance = VK_NULL_HANDLE;

                if (vkCreateInstance(&__info, _create_cb, &_instance) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Instance Builder :: make", "could not create instance"));

                if (volk_load_inst_only) {
                     volkLoadInstanceOnly(_instance);
                     printf(SVKFW_WRAPINFO("VKFW Instance Builder :: make", "only instance-based function pointers are loaded. Call volkLoadDevice() after VkDevice creation.\n"));
                }
            ...
            VkInstanceCreateInfo makeInfoStruct(void *_p_next = nullptr, VkInstanceCreateFlags _flags = 0u) {
        ...
        // Additional methods:
            ...
            void addDMCExtension() {
                if (!allInstanceExtAvailable({{ VK_EXT_DEBUG_UTILS_EXTENSION_NAME }}))
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Instance Builder :: addDMCExtension","debug message callback extension is unavailable"));
    ...
    // Functions definition
        ...
        bool allInstanceLayersAvailable(const StringVec &_layers) {
            ...
            for (const std::string &layerName : _layers.list) {
                ...
                if (!layerFound) {
                    fprintf(outswarn, "%s %s\n", SVKFW_WRAPWARN("VKFWInstance :: allInstanceLayersAvailable","could not find layer"), layerName.c_str());
                    // fflush(outswarn);
        ...
        bool allInstanceExtAvailable(const StringVec &_extensions, const char *_layer) {
            ...
            for (const std::string &extName : _extensions.list) {
                ...
                if (!extFound) {
                    fprintf(outswarn, "%s %s\n", SVKFW_WRAPWARN("VKFWInstance :: allInstanceExtAvailable","could not find extension "), extName.c_str());
                    // fflush(outswarn);
...
//  ============  Vulkan Debug Messenger Callback  ============  \\
    ...
    // Classes definition
        ...
        // Constructors:
            ...
            Builder(const VKFWInstance::BaseHandler &_instance = {}) : instance{_instance.instance} {
        ...
        // Create handlers/CreateInfo struct:
            ...
            void initHandler(Handler &_handle, void *_p_next = nullptr, VkDebugUtilsMessengerCreateFlagsEXT _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                _handle.messenger = make(_p_next, _flags, _create_cb);
                _handle.instance = instance;
                _handle.destroy_callback = _destroy_cb;
            }

            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkDebugUtilsMessengerCreateFlagsEXT _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), instance, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkDebugUtilsMessengerCreateFlagsEXT _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), instance, _destroy_cb };
            }

            VkDebugUtilsMessengerEXT make(void *_p_next = nullptr, VkDebugUtilsMessengerCreateFlagsEXT _flags = 0u,
                                          const VkAllocationCallbacks * _create_cb = nullptr) {
                VkDebugUtilsMessengerCreateInfoEXT __info = makeInfoStruct(_p_next, _flags);
                ...
                if (vkCreateDebugUtilsMessengerEXT(instance, &__info, _create_cb, &_messenger) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW DebugMessenger Builder :: make","could not create debug messenger"));

                return _messenger;
            }

            VkDebugUtilsMessengerCreateInfoEXT makeInfoStruct(void *_p_next = nullptr, VkDebugUtilsMessengerCreateFlagsEXT _flags = 0u) {
        ...
        // Setters:
            ...
            void setInstance(const VKFWInstance::BaseHandler &_instance) {
...
//  ============  Vulkan Physical Device  ============  \\
    ...
    // Classes definition
        ...
        // about one physical device - element of phys_devices vector.
        ...
        // Other methods:
            ...
            void getAllDeviceExtensions(std::vector<VkExtensionProperties> &_extensions, const char *_layerName = nullptr) const {
                return VKFWPhysDevice::getAllDeviceExtensions(phys_device, _extensions, _layerName);
            }

            bool allExtAvailable(const StringVec &_extensions) const {
                return VKFWPhysDevice::allExtAvailable(phys_device, _extensions);
            }

            bool allExtAvailable(const StringVec &_extensions, bool *_flags) const {
                return VKFWPhysDevice::allExtAvailable(phys_device, _extensions, _flags);
            }
        }; // InfoElement END
        ...
        // Constructors:
            ...
            Manager(const VKFWInstance::BaseHandler &_instance = {}) : instance{_instance.instance} {}
            ~Manager() {
                clearManager();
            }
        ...
        // Load properties:
            ...
            void loadInfo() {
                if (instance == VK_NULL_HANDLE)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW PhysDevice Manager :: loadInfo", "instance is not set"));

                uint32_t __devCount = 0;
                vkEnumeratePhysicalDevices(instance, &__devCount, nullptr);

                if (__devCount == 0)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW PhysDevice Manager :: loadInfo", "no GPUs with Vulkan support available"));
        ...
        // Methods to select elements:
            ...
            Handler testPickOne(const Suitability::TestFunc _testFunc) {
                for (uint i = 0u; i < phys_devices.size(); ++i) {
                    if (_testFunc({phys_devices[i], features[i], properties[i]}))
                        return {phys_devices[i]};
                }

                throw std::runtime_error(SVKFW_WRAPERR("VKFW PhysDevice Manager :: testPickOne", "failed to find a suitable GPU"));
                return {};
            }

            // Returns all physical devices for which _testFunc({device, features, properties}) == true
            VecHandler testPickAll(const Suitability::TestFunc _testFunc) {
            ...
            Handler rankPickOne(const Suitability::RankFunc _rankFunc) {
                int __maxRank = 0;
                uint __ind = phys_devices.size();
                for (uint i = 0u; i < phys_devices.size(); ++i) {
                ...
                if (__ind >= phys_devices.size())
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW PhysDevice Manager :: rankPickOne", "failed to find a suitable GPU"));
                return { phys_devices[__ind] };
            }

            // Ranks all physical devices and returns one which passes the comparison to _threshold (default: >=)
            template <VKFWCompareOp COMP_OP = VKFWCompareOp::GE>
            Handler rankPickOne(const Suitability::RankFunc _rankFunc, int _threshold) {
                for (uint i = 0u; i < phys_devices.size(); ++i) {
                    switch (COMP_OP) {
                        case VKFWCompareOp::NE: {
                            if (_rankFunc({phys_devices[i], features[i], properties[i]}) != _threshold)
                                return { phys_devices[i] };
                            break;
                        }
                        case VKFWCompareOp::EQ: {
                            if (_rankFunc({phys_devices[i], features[i], properties[i]}) == _threshold)
                                return { phys_devices[i] };
                            break;
                        }
                        case VKFWCompareOp::LT: {
                            if (_rankFunc({phys_devices[i], features[i], properties[i]}) <  _threshold)
                                return { phys_devices[i] };
                            break;
                        }
                        case VKFWCompareOp::LE: {
                            if (_rankFunc({phys_devices[i], features[i], properties[i]}) <= _threshold)
                                return { phys_devices[i] };
                            break;
                        }
                        case VKFWCompareOp::GE: {
                            if (_rankFunc({phys_devices[i], features[i], properties[i]}) >= _threshold)
                                return { phys_devices[i] };
                            break;
                        }
                        case VKFWCompareOp::GT: {
                            if (_rankFunc({phys_devices[i], features[i], properties[i]}) >  _threshold)
                                return { phys_devices[i] };
                            break;
                        }
                ...
                throw std::runtime_error(SVKFW_WRAPERR("VKFW PhysDevice Manager :: rankPickOne (with threshold)", "failed to find a suitable GPU"));
                return { phys_devices[0] };
            }

            // Ranks all physical devices and returns all which pass the comparison to _threshold (default: >=)
            template <VKFWCompareOp COMP_OP = VKFWCompareOp::GE>
            VecHandler rankPickAll(const Suitability::RankFunc _rankFunc, int _threshold) {
                std::vector<VkPhysicalDevice> __res_devices;
            ...
            template <VKFWCompareOp COMP_OP = VKFWCompareOp::GE>
            VecHandler rankPickAll(const Suitability::RankFunc _rankFunc, std::vector<int> &_ranks, int _threshold) {
                std::vector<VkPhysicalDevice> __res_devices;
...
//  ============  Vulkan Queue Family  ============  \\
    ...
    // Classes definition
        ...
        // Constructors:
            ...
            Manager(const VKFWInstance::BaseHandler  &_instance = {},
                    const VKFWPhysDevice::BaseHandler &_phys_device = {})
                            : instance{_instance.instance}, phys_device{_phys_device.phys_device} {}
            ~Manager() {
                clearManager();
            }
        ...
        // Load properties:
            ...
            void loadInfo() {
                if (instance == VK_NULL_HANDLE)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Queue Family Manager :: loadInfo", "instance is not set"));
                if (phys_device == VK_NULL_HANDLE)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Queue Family Manager :: loadInfo", "physical device is not set"));
        ...
        // Methods to select elements:
            ...
            Handler testPickOne(Suitability::TestFunc _test_func) {
                ...
                fprintf(outswarn, SVKFW_WRAPWARN("VKFW Queue Family Manager :: testPickOne", "could not find queue family"));
                return {uint(-1)};
            }
            ...
            VecHandler testPickAll(Suitability::TestFunc _test_func) {
                ...
                if (__indices.empty())
                    fprintf(outswarn, SVKFW_WRAPWARN("VKFW Queue Family Manager :: testPickAll", "could not find queue family"));
                return { __indices };
            }
            ...
            Handler rankPickOne(Suitability::RankFunc _rank_func) {
                ...
                if (__maxI >= properties.size()) {
                    fprintf(outswarn, SVKFW_WRAPWARN("VKFW Queue Family Manager :: rankPickOne", "could not find queue family"));
                    return {uint(-1)};
                }
                return { __maxI };
            }
            ...
            VecHandler rankPickAll(Suitability::RankFunc _rank_func, int _threshold) {
                ...
                if (__indices.empty())
                    fprintf(outswarn, SVKFW_WRAPWARN("VKFW Queue Family Manager :: rankPickAll", "could not find queue family"));
                return { __indices };
            }
        }; // Manager END
    ...
    namespace VKFWQueue {
        ...
        // Constructors:
            ...
            Builder(const VKFWDevice::BaseHandler &_device = {},
                    const VKFWQueueFamily::BaseHandler &_queue_family = {})
                            : device{_device.device}, queue_family{_queue_family.index} {}
            ~Builder() {
                clearBuilder();
            }
        ...
        // Create handlers/CreateInfo struct:
            ...
            VkQueue make(uint32_t _queue_index) {
        ...
        // Setters:
            ...
            void setDevice(const VKFWDevice::BaseHandler &_device) {
                device = _device.device;
            }

            void setQueueFamily(const VKFWQueueFamily::BaseHandler &_queue_family) {
...
//  ============  Vulkan Logical Device  ============  \\
    ...
    // Functions declaration
        ...
        void checkDeviceExtAvailability(const VKFWPhysDevice::BaseHandler &_phys_device, const StringVec &_extensions, bool *_res, const char *_layer = nullptr);
        void checkDeviceLayerAvailability(const VKFWPhysDevice::BaseHandler &_phys_device, const StringVec &_layers, bool *_res);
        bool allDeviceExtAvailable(const VKFWPhysDevice::BaseHandler &_phys_device, const StringVec &_extensions, const char *_layer = nullptr);
        bool allDeviceLayersAvailable(const VKFWPhysDevice::BaseHandler &_phys_device, const StringVec &_layers);
        uint32_t getAllDeviceExtensions(const VKFWPhysDevice::BaseHandler &_phys_device, std::vector<VkExtensionProperties> &_ext_list, const char *_layer = nullptr);
        uint32_t getAllDeviceValLayers(const VKFWPhysDevice::BaseHandler &_phys_device, std::vector<VkLayerProperties> &_layer_list);
        VkQueue getDeviceQueueHandle(const VKFWDevice::BaseHandler &_device, const VKFWQueueFamily::BaseHandler &_queue_family, uint32_t _queue_index);
        std::vector<VkQueue> getAllDeviceQueueHandles(const VKFWDevice::BaseHandler &_device, const VKFWQueueFamily::BaseHandler &_queue_family, uint32_t _queue_index_offset, uint32_t _queue_index_count);
    ...
    // Classes definition
        ...
        // Constructors:
            ...
            Builder(VKFWPhysDevice::BaseHandler _phys_device = {}) : phys_device{_phys_device.phys_device} {}
            Builder(const Builder &_builder)
                ...
                for (uint i = 0u; i < queue_info.size(); ++i) {
        ...
        // Create handlers/CreateInfo struct:
            ...
            void initHandler(Handler &_handle, void *_p_next = nullptr, VkDeviceCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                _handle.device = make(_p_next, _flags, _create_cb);
                _handle.destroy_callback = _destroy_cb;
            }

            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkDeviceCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkDeviceCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), _destroy_cb };
            }

            VkDevice make(void *_p_next = nullptr, VkDeviceCreateFlags _flags = 0u,
                          const VkAllocationCallbacks * _create_cb = nullptr) {
                if (phys_device == VK_NULL_HANDLE)
                    throw std::logic_error(SVKFW_WRAPERR("VKFW Device Builder :: make", "physical device handle was not set"));

                VkDeviceCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkDevice __device = VK_NULL_HANDLE;

                if (vkCreateDevice(phys_device, &__info, _create_cb, &__device) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Device Builder :: make","could not create device"));
                return __device;
            }

            VkDeviceCreateInfo makeInfoStruct(void *_p_next = nullptr, VkDeviceCreateFlags _flags = 0u) {
        ...
        // Setters:
            ...
            void setPhysicalDevice(VKFWPhysDevice::BaseHandler _phys_device) {
                phys_device = _phys_device.phys_device;
            }
        ...
        // Additional methods:
            ...
            void addQueueInfo(const VkQueueFamilyProperties &_properties, VKFWQueueFamily::BaseHandler _family,
                ...
                if (_queue_count > _properties.queueCount)
                    throw std::invalid_argument(SVKFW_WRAPERR("VKFW Device Builder :: addQueueInfo","_queue_count exceeds the supported number of queues for queue family"));
                if ( (_queue_create_flags & VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT) &&
                    !(_properties.queueFlags & VK_QUEUE_PROTECTED_BIT))
                    throw std::invalid_argument(SVKFW_WRAPERR("VKFW Device Builder :: addQueueInfo","setting protected memory bit when the feature is disabled"));
                ...
                for (uint i = 0u; i < _queue_count; ++i) {
                    if (i >= _priorities.size())
                        __ptr[i] = 1.f;
                    else {
                        __ptr[i] = Func::clamp_GE_LE(_priorities[i], 0.f, 1.f);
                        if (__ptr[i] != _priorities[i])
                            fprintf(outswarn, "%s: %f\n", SVKFW_WRAPWARN("VKFW Device Builder :: addQueueInfo","unnormalized priority value"), _priorities[i]);
                    }
                }
                __newinfo.pQueuePriorities = __ptr;
            }

            void addVecQueueInfo(const std::vector<VkQueueFamilyProperties> &_properties, VKFWQueueFamily::VecHandler _families,
                ...
                if (_properties.size() < _families.count())
                    throw std::invalid_argument(SVKFW_WRAPERR("VKFW Device Builder :: addVecQueueInfo (1)","length of VkQueueFamilyProperties vector is less than queue families count"));

                for (uint i = 0u; i < _families.count(); ++i) {
                    VkDeviceQueueCreateFlags __q_create_flags = 0u;
                    if (i < _queue_create_flags.size())
                        __q_create_flags = _queue_create_flags[i];

                    addQueueInfo(_properties[i], _families.getBaseHandler(i), _queue_count[i],
                                 (i <_priorities.size() ? _priorities[i] : std::vector<float>{1.f}), __q_create_flags);
                }
            }

            void addVecQueueInfo(const VKFWQueueFamily::Manager &_qf_manager, VKFWQueueFamily::VecHandler _families,
                                 const std::vector<uint32_t> &_queue_count, const std::vector<std::vector<float>> &_priorities = {{1.f}},
                                 const std::vector<VkDeviceQueueCreateFlags> &_queue_create_flags = { 0u }) {
                for (uint i = 0u; i < _families.count(); ++i) {
                    VkDeviceQueueCreateFlags __q_create_flags = 0u;
                    ...
                    if (_qf_manager.properties.size() < _families[i])
                        throw std::invalid_argument(SVKFW_WRAPERR("VKFW Device Builder :: addVecQueueInfo (2)",
                                                        "number of VkQueueFamilyProperties in _qf_manager is less than queue family index"));

                    addQueueInfo(_qf_manager.properties[_families[i]], _families.getBaseHandler(i), _queue_count[i],
                                 (i <_priorities.size() ? _priorities[i] : std::vector<float>{1.f}), __q_create_flags);
                }
            }
            ...
            void addValLayers(const StringVec &_val_layers = {{ "VK_LAYER_KHRONOS_validation" }}) {
                if (phys_device == VK_NULL_HANDLE)
                    throw std::logic_error(SVKFW_WRAPERR("VKFW Device Builder :: addValLayers", "physical device handle was not set"));
    ...
    // Functions definition
        ...
        uint32_t getAllDeviceValLayers(const VKFWPhysDevice::BaseHandler &_phys_device, std::vector<VkLayerProperties> &_layer_list) {
            ...
            vkEnumerateDeviceLayerProperties(_phys_device.phys_device, &__layer_count, nullptr);

            _layer_list.resize(__layer_count);
            vkEnumerateDeviceLayerProperties(_phys_device.phys_device, &__layer_count, _layer_list.data());
            return __layer_count;
        }

        uint32_t getAllDeviceExtensions(const VKFWPhysDevice::BaseHandler &_phys_device, std::vector<VkExtensionProperties> &_ext_list, const char *_layer) {
            uint32_t __ext_count;
            vkEnumerateDeviceExtensionProperties(_phys_device.phys_device, _layer, &__ext_count, nullptr);

            _ext_list.resize(__ext_count);
            vkEnumerateDeviceExtensionProperties(_phys_device.phys_device, _layer, &__ext_count, _ext_list.data());
            return __ext_count;
        }
        ...
        bool allDeviceLayersAvailable(const VKFWPhysDevice::BaseHandler &_phys_device, const StringVec &_layers) {
            ...
            for (const std::string &layer_name : _layers.list) {
                ...
                if (!__layer_found) {
                    fprintf(outswarn, "%s %s\n", SVKFW_WRAPWARN("VKFWDevice :: allDeviceLayersAvailable","could not find layer"), layer_name.c_str());
                    // fflush(outswarn);
        ...
        bool allDeviceExtAvailable(const VKFWPhysDevice::BaseHandler &_phys_device, const StringVec &_extensions, const char *_layer) {
            ...
            for (const std::string &ext_name : _extensions.list) {
                ...
                if (!__ext_found) {
                    fprintf(outswarn, "%s %s\n", SVKFW_WRAPWARN("VKFWDevice :: allDeviceExtAvailable","could not find extension"), ext_name.c_str());
                    // fflush(outswarn);
        ...
        // for each extension name in _extensions returns if it is available
        // (_res must have at least the same length (in elements) as _extensions)
        void checkDeviceExtAvailability(const VKFWPhysDevice::BaseHandler &_phys_device, const StringVec &_extensions,
        ...
        VkQueue getDeviceQueueHandle(const VKFWDevice::BaseHandler &_device,
                                     const VKFWQueueFamily::BaseHandler &_queue_family, uint32_t _queue_index) {
            VkQueue __res = VK_NULL_HANDLE;
            vkGetDeviceQueue(_device.device, _queue_family.index, _queue_index, &__res);

            return __res;
        }

        std::vector<VkQueue> getAllDeviceQueueHandles(const VKFWDevice::BaseHandler &_device,
                                                      const VKFWQueueFamily::BaseHandler &_queue_family,
            ...
            for (uint32_t i = 0u; i < __res.size(); ++i)
                vkGetDeviceQueue(_device.device, _queue_family.index, i + _queue_index_offset, &__res[i]);
...
//  ============  Vulkan Surface  ============  \\
    ...
    namespace VKFWSurfaceKHR {
        struct Builder { // Although GLFW covers all portability, TODO: add "canonical" Builder for VkSurfaceKHR
        // Data:
        ...
        // Constructors & clearBuilder

            Builder(const VKFWInstance::BaseHandler &_instance = {})
                            : instance{_instance.instance} {}
            Builder(const VKFWInstance::BaseHandler &_instance, const Window &_glfw_window)
                            : instance{_instance.instance}, glfw_window{_glfw_window.window} {}
        ...
        // Create handlers/CreateInfo struct:
            ...
            void initHandler(Handler &_handle, void *_p_next = nullptr, VkFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                _handle.surface = make(_p_next, _flags, _create_cb);
                _handle.instance = instance;
                _handle.destroy_callback = _destroy_cb;
            }

            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), instance, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), instance, _destroy_cb };
            }

            VkSurfaceKHR make(void *_p_next = nullptr, VkFlags _flags = 0u, // these two parameters are not used
                              const VkAllocationCallbacks * _create_cb = nullptr) {
                VkSurfaceKHR __surface = VK_NULL_HANDLE;
                if (glfwCreateWindowSurface(instance, glfw_window, _create_cb, &__surface) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Surface Builder :: make","could not create surface"));
                return __surface;
            }
...
//  ============ Vulkan Swapchain  ============  \\
    ...
    // Classes definition
        ...
        // Constructors:
            ...
            Builder(VKFWPhysDevice::BaseHandler _phys_device = {},
                    const VKFWSurfaceKHR::BaseHandler &_surface = {},
                    const VKFWDevice::BaseHandler &_device = {})
                            : phys_device{_phys_device.phys_device},
                              surface{_surface.surface}, device{_device.device} {
        ...
        // Create handlers/CreateInfo struct:
            ...
            // use makeBaseHandler() or clearHandler()
            void initHandler(Handler &_handle, void *_p_next = nullptr, VkSwapchainCreateFlagsKHR _flags = 0u,
                             const VkAllocationCallbacks * _create_cb = nullptr,
                             const VkAllocationCallbacks *_destroy_cb = nullptr) {
                if (_handle.swapchain != VK_NULL_HANDLE)
                    setOldSwapchainWithDevice(_handle);

                _handle.swapchain = make(_p_next, _flags, _create_cb);
                _handle.device = device;
                _handle.destroy_callback = _destroy_cb;
            }

            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkSwapchainCreateFlagsKHR _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkSwapchainCreateFlagsKHR _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkSwapchainKHR make(void *_p_next = nullptr, VkSwapchainCreateFlagsKHR _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr) {
                VkSwapchainCreateInfoKHR __info = makeInfoStruct(_p_next, _flags);
                ...
                if (vkCreateSwapchainKHR(device, &__info, _create_cb, &__swapchain) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Swapchain Builder :: make","could not create swapchain"));
                return __swapchain;
            }

            VkSwapchainCreateInfoKHR makeInfoStruct(void *_p_next = nullptr, VkSwapchainCreateFlagsKHR _flags = 0u) {
        ...
        // Setters:
            ...
            void setPhysicalDevice(VKFWPhysDevice::Handler _phys_device) {
                phys_device = _phys_device.phys_device;
            }

            void setDevice(const VKFWDevice::Handler &_device) {
                device = _device.device;
            }
            ...
            void setOldSwapchainWithDevice(Handler &_swapchain) {
                old_swapchain = _swapchain.swapchain;
                device = _swapchain.device;
                _swapchain.clearHandler();
            }

            void setSurface(const VKFWSurfaceKHR::Handler &_surface) {
            ...
            void setQueueFaimilies(const VKFWQueueFamily::VecHandler &_q_families) {
                queue_families = _q_families.indices;
            }
        ...
        // Additional methods:
            ...
            // Must be called before most other methods. Called in constructor if non-empty handlers are passed
            void loadSurfaceCapabilities(VKFWPhysDevice::Handler _phys_device = VK_NULL_HANDLE,
                                         const VKFWSurfaceKHR::Handler &_surface = { VK_NULL_HANDLE, VK_NULL_HANDLE }) {
                if (_phys_device.phys_device != VK_NULL_HANDLE)
                    phys_device = _phys_device.phys_device;
                if (_surface.surface != VK_NULL_HANDLE)
                    surface = _surface.surface;
            ...
            VkImageUsageFlags checkAndSetImageUsage(VkImageUsageFlags _usage) {
                ...
                if (__res == _usage)
                    ...
                    fprintf(outswarn, SVKFW_WRAPWARN("VKFWSwapchainKHR Builder :: checkAndSetImageUsage",
                                                     "some usage flags are not supported\n"));
                return __res;
            }

            void chooseImageCount(uint32_t _count) {
                image_count = Func::clamp_GE_LE(_count,
                                                capabilities.minImageCount,
                                                capabilities.maxImageCount ? // (== 0) means no max
                                                capabilities.maxImageCount : uint32_t(-1));

                if (image_count != _count)
                    fprintf(outswarn, SVKFW_WRAPWARN("VKFW Swapchain Builder :: chooseImageCount",
                ...
                else printf(SVKFW_WRAPINFO("VKFW Swapchain Builder :: chooseImageCount", "imageCount: %d, the interval: [%d, %d]\n"),
            ...
            bool chooseExtent(UVec2 _extent) { // TODO: either more info from functions (like this bool) or revert
                ...
                if (capabilities.currentExtent.width  != uint32_t(-1) &&
                    ...
                    image_info.extent = { Func::clamp_GE_LE(_extent.x,
                                            capabilities.minImageExtent.width,
                                            capabilities.maxImageExtent.width),
                                          Func::clamp_GE_LE(_extent.y,
                                            capabilities.minImageExtent.height,
                                            capabilities.maxImageExtent.height)  };
                    printf(SVKFW_WRAPINFO("VKFW Swapchain Builder :: chooseExtent (1)", "resulting swap extent: [%d, %d]\n"),
            ...
            void chooseCurrentExtent(const Window &_window) {
                if (capabilities.currentExtent.width != uint32_t(-1)) {
                    image_info.extent = capabilities.currentExtent;
                    printf(SVKFW_WRAPINFO("VKFW Swapchain Builder :: chooseExtent (2)", "from window's current swap extent: [%d, %d]\n"),
                                            image_info.extent.width, image_info.extent.height);
                    return;
                }
                else {
                    ...
                    image_info.extent = { Func::clamp_GE_LE(__fbsize.x,
                                            capabilities.minImageExtent.width,
                                            capabilities.maxImageExtent.width),
                                          Func::clamp_GE_LE(__fbsize.y,
                                            capabilities.minImageExtent.height,
                                            capabilities.maxImageExtent.height)  };
                    printf(SVKFW_WRAPINFO("VKFW Swapchain Builder :: chooseExtent (2)", "from framebuffer size: [%d, %d]\n"),
                                            image_info.extent.width, image_info.extent.height);
                }
            }
            ...
            void chooseFormat(const Suitability::FormatTestFunc _test_func = Suitability::defaultFormatTest) {
                ...
                for (VkSurfaceFormatKHR __format : __formats) {
                    if (_test_func(__format)) {
                        ...
                        printf(SVKFW_WRAPINFO("VKFW Swapchain Builder :: chooseFormat (1)", "format: %d, color space: %d\n"), uint(__format.format), uint(__format.colorSpace));
                        return;
                    }
                }
                throw std::runtime_error(SVKFW_WRAPERR("VKFW Swapchain Builder :: chooseFormat (1)", "could not choose format"));
            }
            ...
            void chooseFormat(const Suitability::FormatRankFunc _rank_func) {
                ...
                if (__curr_ind > __formats.size())
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Swapchain Builder :: chooseFormat (2)", "could not choose format"));
                printf(SVKFW_WRAPINFO("VKFW Swapchain Builder :: chooseFormat (2)", "format: %d, color space: %d\n"),
                        uint(__formats[__curr_ind].format), uint(__formats[__curr_ind].colorSpace));
            ...
            bool choosePresentationMode(const Suitability::PresentModeTestFunc _test_func = Suitability::defaultPresentModeTest) {
                ...
                for (VkPresentModeKHR __mode : __pres_modes)
                    if (_test_func(__mode)) {
                        present_mode = __mode;
                        printf(SVKFW_WRAPINFO("VKFW Swapchain Builder :: choosePresentationMode (1)", "mode %d\n"), uint(__mode));
                        return true;
                    }

                fprintf(outswarn, SVKFW_WRAPWARN("VKFW Swapchain Builder :: choosePresentationMode (1)",
            ...
            void choosePresentationMode(const Suitability::PresentModeRankFunc _rank_func) {
                ...
                printf(SVKFW_WRAPINFO("VKFW Swapchain Builder :: choosePresentationMode (2)", "mode %d\n"), uint(__curr_mode));
    ...
    // Functions definition
        ...
        std::vector<VkSurfaceFormatKHR> getAllFormats(VkPhysicalDevice _phys_device, const VkSurfaceKHR &_surface) {
            ...
            if (__format_count != 0u) {
                ...
                printf(SVKFW_WRAPINFO("VKFW Swapchain :: getAllFormats", "did not find surface formats.\n"));
            }
            return __formats;
        }
        ...
        std::vector<VkPresentModeKHR> getAllPresentationModes(VkPhysicalDevice _phys_device, const VkSurfaceKHR &_surface) {
            ...
            if (__pres_mode_count != 0) {
                ...
                printf(SVKFW_WRAPINFO("VKFW Swapchain :: getAllPresentationModes", "did not find presentation modes.\n"));
            }
            return __pres_modes;
        }
...
//  ============  Vulkan Image View  ============  \\
    ...
    namespace VKFWImageView {
        ...
        // Constructors:
            ...
            Builder(const VKFWDevice::BaseHandler &_device = {}, const VKFWImage::BaseHandler &_image = {})
                            : device{_device.device}, image{_image.image} {}
            ~Builder() {
                clearBuilder();
            }
        ...
        // Create handlers/CreateInfo struct:
            ...
            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkImageViewCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkImageViewCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkImageView make(void *_p_next = nullptr, VkImageViewCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr) {
                VkImageViewCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkImageView __image_view = VK_NULL_HANDLE;
                if (vkCreateImageView(device, &__info, _create_cb, &__image_view) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Image View Builder :: make",
                                                            "failed to create image view"));

                return __image_view;
            }

            VkImageViewCreateInfo makeInfoStruct(void *_p_next = nullptr, VkImageViewCreateFlags _flags = 0u) {
        ...
        // Setters:
            ...
            void setImage(const VKFWImage::BaseHandler &_image) {
                image = _image.image;
            }
...
//  ============  Vulkan Shader Module  ============  \\
    ...
    namespace VKFWShaderModule {
        ...
        // Constructors:
            ...
            Builder(const VKFWDevice::BaseHandler &_device = {}) : device{_device.device} {}
            Builder(const char *_shader_fpath, const VKFWDevice::BaseHandler &_device) : device{_device.device} {
                fromFile(_shader_fpath);
            }
            Builder(const char *_buffer_str, size_t _buffer_size, const VKFWDevice::BaseHandler &_device) : device{_device.device} {
        ...
        // Create handlers/CreateInfo struct:
            ...
            void initHandler(Handler &_handle, void *_p_next = nullptr, VkShaderModuleCreateFlags _flags = 0u,
                             const VkAllocationCallbacks * _create_cb = nullptr,
                             const VkAllocationCallbacks *_destroy_cb = nullptr) {
                _handle.shader_module = make(_p_next, _flags, _create_cb);
                _handle.device = device;
                _handle.destroy_callback = _destroy_cb;
            }

            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkShaderModuleCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkShaderModuleCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkShaderModule make(void *_p_next = nullptr, VkShaderModuleCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr) {
                VkShaderModuleCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkShaderModule __shader = VK_NULL_HANDLE;
                if (vkCreateShaderModule(device, &__info, _create_cb, &__shader) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Shader Module Builder :: make",
                                                            "failed to create shader module"));

                return __shader;
            }

            VkShaderModuleCreateInfo makeInfoStruct(void *_p_next = nullptr, VkShaderModuleCreateFlags _flags = 0u) {
        ...
        // Additional methods:
            ...
            void fromFile(const char *_fpath) {
                ...
                if (!__shader_file.is_open()) {
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Shader Module Builder :: fromFile",
            ...
            void fromString(const char *_sh_str, size_t _length = 0u) {
                if (_sh_str == nullptr)
                    throw std::invalid_argument(SVKFW_WRAPERR("VKFW Shader Module Builder :: fromString (1)", "no string provided"));
                if (_length == 0u) {
                    while (_sh_str[_length] != '\0') ++_length;
                    if (_length == 0u)
                        throw std::invalid_argument(SVKFW_WRAPERR("VKFW Shader Module Builder :: fromString (1)", "string is empty"));
                }
            ...
            void fromString(const std::string &_str_buffer) {
                if (_str_buffer.length() == 0u)
                    throw std::invalid_argument(SVKFW_WRAPERR("VKFW Shader Module Builder :: fromString (2)", "string is empty"));
...
//  ============  Vulkan Specialization Info  ============  \\
    ...
    namespace VKFWSpecializationInfo {

    // TODO: Make InfoHandler (like Handler/BaseHandler, but for all Vulkan Info structures), with their functions inverted:
    //       InfoBaseHandler destroys all internal array/pointer members (NOT RECURSIVELY), and InfoHandler : InfoBaseHandler has a shallow copy.
    // Then replace all this with SctHandler (Struct handler): a handler for any Vulkan struct that has sType member


        struct InfoBaseHandler {
        // Data:
        ...
        // Constructors:
            ...
            InfoBaseHandler() {}
            InfoBaseHandler(const VkSpecializationInfo &_spec_info) : spec_info{_spec_info} {}
            InfoBaseHandler(const InfoBaseHandler &_handler) : spec_info{_handler.spec_info} {}
            InfoBaseHandler(InfoBaseHandler &&_handler) : spec_info{_handler.spec_info} {
                _handler.clearHandler();
            }
            virtual ~InfoBaseHandler() {
        ...
        // Delete pointer members:
            ...
            void deletePointerMembers() {
                ...
                if (spec_info.dataSize && spec_info.pData != nullptr)
                    delete[] (const char*) spec_info.pData; // TODO: using new char[] is required for InfoBaseHandler
            }
        ...
        // Assignment:
            ...
            InfoBaseHandler& operator=(const InfoBaseHandler &_handler) {
                spec_info = _handler.spec_info;
                return *this;
            }

            InfoBaseHandler& operator=(InfoBaseHandler &&_handler) {
                spec_info = _handler.spec_info;
                _handler.clearHandler();
                return *this;
            }
        ...
        struct InfoHandler : InfoBaseHandler {
        // Constructors:

            InfoHandler() {}
            InfoHandler(const VkSpecializationInfo &_spec_info) : InfoBaseHandler{_spec_info} {}
            InfoHandler(const InfoHandler &_handler) : InfoBaseHandler{_handler.spec_info} {}
            InfoHandler(InfoHandler &&_handler) : InfoBaseHandler{_handler.spec_info} {
                _handler.clearHandler();
            }
            ~InfoHandler() {
                clearHandler(); // All the data is shallow-copied => nullifying all pointers, so no "delete[]" is called in base destructor
            }
        ...
        // Data:
            ...
            std::vector<unsigned char> data; // TODO: CHECK, CHECK, CHECK!
        ...
        // Create handlers/CreateInfo struct:
            ...
            // Has deep copied arrays, which are deleted in InfoBaseHandler upon destruction
            InfoBaseHandler makeInfoBaseHandler() {
            ...
            inline InfoHandler makeInfoHandler() {
        ...
        // Additional methods:
            ...
            void addEntry(uint32_t _const_id, void *_data, size_t _size_bytes) {
                if (data.size() > uint32_t(-1)) // hypothetical error
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Specialization Info :: addEntry", "data size makes offset invalid"));
...
//  ================================================  \\
    ...
    namespace VKFWAttachmentDescription {
        struct SctBasic {
        // Data:
    ...
    namespace VKFWSubpassDescription {
        struct SctBaseHandler {
        // Data:

            VkSubpassDescription subpass_description{};


        // Constructors/clearHandler:

            SctBaseHandler() {}
            SctBaseHandler(const VkSubpassDescription &_subpass_description) : subpass_description{_subpass_description} {}
            SctBaseHandler(const SctBaseHandler &_handler) : subpass_description{_handler.subpass_description} {}
            SctBaseHandler(SctBaseHandler &&_handler) : subpass_description{_handler.subpass_description} {
                _handler.clearHandler();
            }
            virtual ~SctBaseHandler() {
                deletePointerMembers();
                clearHandler();
            }


        // Delete array/pointer members:
        ...
        // Assignment:
            ...
            SctBaseHandler& operator=(const SctBaseHandler &_handler) {
                subpass_description = _handler.subpass_description;
                return *this;
            }

            SctBaseHandler& operator=(SctBaseHandler &&_handler) {
                subpass_description = _handler.subpass_description;
                _handler.clearHandler();
                return *this;
            }
        ...
        struct SctHandler : SctBaseHandler {
        // Constructors:

            SctHandler() {}
            SctHandler(const VkSubpassDescription &_subpass_description) : SctBaseHandler{_subpass_description} {}
            SctHandler(const SctHandler &_handler) : SctBaseHandler{_handler.subpass_description} {}
            SctHandler(SctHandler &&_handler) : SctBaseHandler{_handler.subpass_description} {
                _handler.clearHandler();
            }
            ~SctHandler() {
                clearHandler();
    ...
    namespace VKFWSubpassDependency {
        struct SctBasic {
        // Data:
...
//  ============  Vulkan Render Pass  ============  \\
    ...
    namespace VKFWRenderPass {
        ...
        // Create handlers/CreateInfo struct:
            ...
            void initHandler(Handler &_handle, void *_p_next = nullptr, VkRenderPassCreateFlags _flags = 0u,
                             const VkAllocationCallbacks * _create_cb = nullptr,
                             const VkAllocationCallbacks *_destroy_cb = nullptr) {
                _handle.render_pass = make(_p_next, _flags, _create_cb);
                _handle.device = device;
                _handle.destroy_callback = _destroy_cb;
            }

            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkRenderPassCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkRenderPassCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkRenderPass make(void *_p_next = nullptr, VkRenderPassCreateFlags _flags = 0u,
                              const VkAllocationCallbacks * _create_cb = nullptr) {
                VkRenderPassCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkRenderPass __render_pass = VK_NULL_HANDLE;
                if (vkCreateRenderPass(device, &__info, _create_cb, &__render_pass) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Render Pass Builder :: make",
                                                            "failed to create render pass"));

                return __render_pass;
            }

            VkRenderPassCreateInfo makeInfoStruct(void *_p_next = nullptr, VkRenderPassCreateFlags _flags = 0u) {
        ...
        // Setters:
            ...
            void setAttachments(const std::vector<VKFWAttachmentDescription::SctBasic> &_attachments) {
            ...
            // TODO: VKFWSubpassDescription::SctBaseVecHandler

            // void setSubpasses(const std::vector<VKFWSubpassDescription::SctBaseHandler> &_subpasses) {
            //     subpasses.resize(_subpasses.size());
            //     for (uint32_t i = 0u; i < _subpasses.size(); ++i)
            //         subpasses[i] = _subpasses[i].subpass_description;
            // }

            // void setSubpasses(const std::vector<VKFWSubpassDescription::SctHandler> &_subpasses) {
            //     subpasses.resize(_subpasses.size());
            //     for (uint32_t i = 0u; i < _subpasses.size(); ++i)
            //         subpasses[i] = _subpasses[i].subpass_description;
            // }
...
//  ============  Vulkan Pipeline Layout  ============  \\
    ...
    namespace VKFWPipelineLayout {
        ...
        // Create handlers/CreateInfo struct:
            ...
            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkPipelineLayoutCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkPipelineLayoutCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkPipelineLayout make(void *_p_next = nullptr, VkPipelineLayoutCreateFlags _flags = 0u,
                                  const VkAllocationCallbacks * _create_cb = nullptr) {
                VkPipelineLayoutCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkPipelineLayout __layout = VK_NULL_HANDLE;
                if (vkCreatePipelineLayout(device, &__info, _create_cb, &__layout) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Pipeline Layout Builder :: make", "could not create pipeline layout"));

                return __layout;
            }

            VkPipelineLayoutCreateInfo makeInfoStruct(void *_p_next = nullptr, VkPipelineLayoutCreateFlags _flags = 0u) {
...
//  ============  Pipeline Shader Stage  ============  \\
        ...
        namespace ShaderStage {
            // For now it requires all VKFWSpecializationInfo::InfoBaseHandler and ShaderStage::Builder
            // to exist until the creation of pipeline
            // (Builder - for VkSpecializationInfo*, VKFWSpecializationInfo handler - for its pointers/arrays)
            ...
            // Constructors/clearHandler:
                ...
                InfoBaseHandler() {}
                InfoBaseHandler(const VkPipelineShaderStageCreateInfo &_shader_stage_create_info) : shader_stage_create_info{_shader_stage_create_info} {}
                InfoBaseHandler(const InfoBaseHandler &_handler) : shader_stage_create_info{_handler.shader_stage_create_info} {}
                InfoBaseHandler(InfoBaseHandler &&_handler) : shader_stage_create_info{_handler.shader_stage_create_info} {
                    _handler.clearHandler();
                }
                virtual ~InfoBaseHandler() {
                    deletePointerMembers();
            ...
            // Constructors:
                ...
                InfoHandler() {}
                InfoHandler(const VkPipelineShaderStageCreateInfo &_shader_stage_create_info) : InfoBaseHandler{_shader_stage_create_info} {}
                InfoHandler(const InfoHandler &_handler) : InfoBaseHandler{_handler.shader_stage_create_info} {}
                InfoHandler(InfoHandler &&_handler) : InfoBaseHandler{_handler.shader_stage_create_info} {
                    _handler.clearHandler();
                }
                ~InfoHandler() {
                    clearHandler();
            ...
            // TODO: InfoBaseVecHandler - deletes pointer members in destructor for each element of vector,
            // v     and InfoVecHandler destructor clears before that. This is the first InfoBaseVecHandler - complete it and use as a template.
            ...
            using InfoBaseVecHandler = InfoVecHandler;
            ...
            // Setters:
                ...
                void setShaderInfo(const VKFWShaderModule::BaseHandler &_shader, VkShaderStageFlagBits _stage, const char *_name) {
                    shader_module = _shader.shader_module;
                    stage = _stage;
...
//  ============  Pipeline Dynamic State  ============  \\
        ...
        namespace DynamicState {
            ...
            // Constructors/clearHandler:
                ...
                InfoBaseHandler() {}
                InfoBaseHandler(const VkPipelineDynamicStateCreateInfo &_dynamic_state_create_info)
                                : dynamic_state_create_info{_dynamic_state_create_info} {}
                InfoBaseHandler(const InfoBaseHandler &_handler) : dynamic_state_create_info{_handler.dynamic_state_create_info} {}
                InfoBaseHandler(InfoBaseHandler &&_handler) : dynamic_state_create_info{_handler.dynamic_state_create_info} {
                    _handler.clearHandler();
                }
                virtual ~InfoBaseHandler() {
                    deletePointerMembers();
            ...
            // Constructors:
                ...
                InfoHandler() {}
                InfoHandler(const VkPipelineDynamicStateCreateInfo &_dynamic_state_create_info) : InfoBaseHandler{_dynamic_state_create_info} {}
                InfoHandler(const InfoHandler &_handler) : InfoBaseHandler{_handler.dynamic_state_create_info} {}
                InfoHandler(InfoHandler &&_handler) : InfoBaseHandler{_handler.dynamic_state_create_info} {
                    _handler.clearHandler();
                }
                ~InfoHandler() {
                    clearHandler();
            ...
            // Additional methods:
                ...
                bool delDynamicState(VkDynamicState _state) {
                    for (uint i = 0u; i < dynamic_states.size(); ++i) {
...
//  ============  Pipeline Vertex Input State  ============  \\
        ...
        namespace VertexInputState {
            ...
            // Constructors/clearHandler:
                ...
                InfoBaseHandler() {}
                InfoBaseHandler(const VkPipelineVertexInputStateCreateInfo &_vertex_input_state_create_info)
                                : vertex_input_state_create_info{_vertex_input_state_create_info} {}
                InfoBaseHandler(const InfoBaseHandler &_handler) : vertex_input_state_create_info{_handler.vertex_input_state_create_info} {}
                InfoBaseHandler(InfoBaseHandler &&_handler) : vertex_input_state_create_info{_handler.vertex_input_state_create_info} {
                    _handler.clearHandler();
                }
                virtual ~InfoBaseHandler() {
                    deletePointerMembers();
            ...
            // Constructors:
                ...
                InfoHandler() {}
                InfoHandler(const VkPipelineVertexInputStateCreateInfo &_vertex_input_state_create_info) : InfoBaseHandler{_vertex_input_state_create_info} {}
                InfoHandler(const InfoHandler &_handler) : InfoBaseHandler{_handler.vertex_input_state_create_info} {}
                InfoHandler(InfoHandler &&_handler) : InfoBaseHandler{_handler.vertex_input_state_create_info} {
                    _handler.clearHandler();
                }
                ~InfoHandler() {
                    clearHandler();
...
//  ============  Pipeline Viewport State  ============  \\
        ...
        namespace ViewportState {
            ...
            // Constructors/clearHandler:
                ...
                InfoBaseHandler() {}
                InfoBaseHandler(const VkPipelineViewportStateCreateInfo &_viewport_state_create_info)
                                : viewport_state_create_info{_viewport_state_create_info} {}
                InfoBaseHandler(const InfoBaseHandler &_handler) : viewport_state_create_info{_handler.viewport_state_create_info} {}
                InfoBaseHandler(InfoBaseHandler &&_handler) : viewport_state_create_info{_handler.viewport_state_create_info} {
                    _handler.clearHandler();
                }
                virtual ~InfoBaseHandler() {
                    deletePointerMembers();
            ...
            // Constructors:
                ...
                InfoHandler() {}
                InfoHandler(const VkPipelineViewportStateCreateInfo &_viewport_state_create_info) : InfoBaseHandler{_viewport_state_create_info} {}
                InfoHandler(const InfoHandler &_handler) : InfoBaseHandler{_handler.viewport_state_create_info} {}
                InfoHandler(InfoHandler &&_handler) : InfoBaseHandler{_handler.viewport_state_create_info} {
                    _handler.clearHandler();
                }
                ~InfoHandler() {
                    clearHandler();
            ...
            // Setters:
                ...
                void setViewports(const std::vector<VKFWViewport::SctBasic> &_viewports) {
                    viewports.resize(_viewports.size());
                    for (uint i = 0u; i < _viewports.size(); ++i)
                        viewports[i] = _viewports[i].viewport;
                }

                void setScissors(const std::vector<VKFWRect2D::SctBasic> &_scissors) {
                    scissors.resize(_scissors.size());
                    for (uint i = 0u; i < _scissors.size(); ++i)
            ...
            // Additional methods:
                ...
                void addViewports(const std::vector<VKFWViewport::SctBasic> &_viewports) {
                ...
                void addScissors(const std::vector<VKFWRect2D::SctBasic> &_scissors) {
...
//  ============  Pipeline Color Blend State  ============  \\
        ...
        namespace ColorBlendState {
            ...
            // Constructors:
                ...
                InfoBaseHandler() {}
                InfoBaseHandler(const VkPipelineColorBlendStateCreateInfo &_color_blend_state_create_info)
                                : color_blend_state_create_info{_color_blend_state_create_info} {}
                InfoBaseHandler(const InfoBaseHandler &_handler) : color_blend_state_create_info{_handler.color_blend_state_create_info} {}
                InfoBaseHandler(InfoBaseHandler &&_handler) : color_blend_state_create_info{_handler.color_blend_state_create_info} {
                    _handler.clearHandler();
                }
                ~InfoBaseHandler() {
                    deletePointerMembers();
            ...
            // Constructors:
                ...
                InfoHandler() {}
                InfoHandler(const VkPipelineColorBlendStateCreateInfo &_color_blend_state_create_info) : InfoBaseHandler{_color_blend_state_create_info} {}
                InfoHandler(const InfoHandler &_handler) : InfoBaseHandler{_handler.color_blend_state_create_info} {}
                InfoHandler(InfoHandler &&_handler) : InfoBaseHandler{_handler.color_blend_state_create_info} {
                    _handler.clearHandler();
                }
                ~InfoHandler() {
                    clearHandler();
...
//  ============  Vulkan Graphics Pipeline  ============  \\
        ...
        namespace Graphics {
            ...
            // Constructors:
                ...
                Builder(const VKFWDevice::BaseHandler &_device = {},
                        const VKFWPipelineLayout::BaseHandler &_layout = {},
                        const VKFWPipelineCache::BaseHandler &_cache = {})
                                : device{_device.device}, layout{_layout.pipeline_layout},
                                  pipeline_cache{_cache.pipeline_cache} {}
                ~Builder() {
                    clearBuilder();
                }
            ...
            // Create handlers/CreateInfo struct:
                ...
                BaseHandler makeBaseHandler(void *_p_next = nullptr, VkPipelineCreateFlags _flags = 0u,
                                            const VkAllocationCallbacks * _create_cb = nullptr,
                                            const VkAllocationCallbacks *_destroy_cb = nullptr) {
                    return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
                }

                Handler makeHandler(void *_p_next = nullptr, VkPipelineCreateFlags _flags = 0u,
                                    const VkAllocationCallbacks * _create_cb = nullptr,
                                    const VkAllocationCallbacks *_destroy_cb = nullptr) {
                    return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
                }

                VkPipeline make(void *_p_next = nullptr, VkPipelineCreateFlags _flags = 0u,
                          const VkAllocationCallbacks * _create_cb = nullptr) {
                    VkGraphicsPipelineCreateInfo __info = makeInfoStruct(_p_next, _flags);
                    ...
                    if (vkCreateGraphicsPipelines(device, pipeline_cache, 1, &__info, _create_cb, &__pipeline) != VK_SUCCESS)
                        throw std::runtime_error(SVKFW_WRAPERR("VKFW Graphics Pipeline :: make", "failed to create graphics pipeline"));

                    return __pipeline;
                }

                VkGraphicsPipelineCreateInfo makeInfoStruct(void *_p_next = nullptr, VkPipelineCreateFlags _flags = 0u) {
            ...
            // Setters:
                ...
                void setRenderPassWithSubpass(const VKFWRenderPass::BaseHandler &_render_pass, uint32_t _subpass) {
                    render_pass = _render_pass.render_pass;
                    subpass = _subpass;
                }

                void setBasePipeline(int32_t _base_pipeline_index = 0, const VKFWPipeline::BaseHandler &_base_pipeline_handle = {}) {
                    base_pipeline_handle = _base_pipeline_handle.pipeline;
                    base_pipeline_index = _base_pipeline_index;
                }

                void setPipelineLayout(const VKFWPipelineLayout::BaseHandler &_layout) {
                    layout = _layout.pipeline_layout;
                }

                void setPipelineCache(const VKFWPipelineCache::BaseHandler &_cache) {
                    pipeline_cache = _cache.pipeline_cache;
                }
...
//  ============  Vulkan Compute Pipeline  ============  \\
        ...
        namespace Compute {
            ...
            // Create handlers/CreateInfo struct:
                ...
                VkPipeline make(void *_p_next = nullptr, VkPipelineCreateFlags _flags = 0u,
                          const VkAllocationCallbacks * _create_cb = nullptr) {
                    VkComputePipelineCreateInfo __info = makeInfoStruct(_p_next, _flags);
                    VkPipeline __pipeline = VK_NULL_HANDLE;
                    if (vkCreateComputePipelines(device, pipeline_cache, 1, &__info, _create_cb, &__pipeline) != VK_SUCCESS)
                        throw std::runtime_error(SVKFW_WRAPERR("VKFW Compute Pipeline :: make", "failed to create compute pipeline"));

                    return __pipeline;
                }

                VkComputePipelineCreateInfo makeInfoStruct(void *_p_next = nullptr, VkPipelineCreateFlags _flags = 0u) {
...
//  ============  Vulkan Framebuffer  ============  \\
    ...
    namespace VKFWFramebuffer {
        ...
        // Constructors:
            ...
            Builder(const VKFWDevice::BaseHandler &_device = {}, const VKFWRenderPass::BaseHandler &_render_pass = {})
                            : device{_device.device}, render_pass{_render_pass.render_pass} {}
            ~Builder() {
                clearBuilder();
            }
        ...
        // Create Handlers/CreateInfo struct:
            ...
            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkFramebufferCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkFramebufferCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkFramebuffer make(void *_p_next = nullptr, VkFramebufferCreateFlags _flags = 0u,
                        const VkAllocationCallbacks * _create_cb = nullptr) {
                VkFramebufferCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkFramebuffer __framebuffer = VK_NULL_HANDLE;
                if (vkCreateFramebuffer(device, &__info, _create_cb, &__framebuffer) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Framebuffer :: make", "failed to create framebuffer"));

                return __framebuffer;
            }

            VkFramebufferCreateInfo makeInfoStruct(void *_p_next = nullptr, VkFramebufferCreateFlags _flags = 0u) {
        ...
        // Setters:
            ...
            void setRenderPass(const VKFWRenderPass::BaseHandler &_render_pass) {
                render_pass = _render_pass.render_pass;
            }
            ...
            void setAttachments(const VKFWImageView::BaseVecHandler &_attachments) {
                attachments = _attachments.image_views;
            }

            void setAttachment(const VKFWImageView::BaseHandler &_attachment) {
                attachments = { _attachment.image_view };
            }
...
// ============  Vulkan Command Pool  ============  \\
    ...
    namespace VKFWCommandPool {
        ...
        // Constructors:
            ...
            Builder(const VKFWDevice::BaseHandler &_device = {}, const VKFWQueueFamily::BaseHandler &_queue_family = {})
                            : device{_device.device}, queue_family{_queue_family.index} {}
            ~Builder() {
                clearBuilder();
            }
        ...
        // Create Handlers/CreateInfo struct:
            ...
            Handler makeHandler(void *_p_next = nullptr, VkCommandPoolCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkCommandPool make(void *_p_next = nullptr, VkCommandPoolCreateFlags _flags = 0u,
                        const VkAllocationCallbacks * _create_cb = nullptr) {
                VkCommandPoolCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkCommandPool __framebuffer = VK_NULL_HANDLE;
                if (vkCreateCommandPool(device, &__info, _create_cb, &__framebuffer) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Command Pool :: make", "failed to create command pool"));

                return __framebuffer;
            }

            VkCommandPoolCreateInfo makeInfoStruct(void *_p_next = nullptr, VkCommandPoolCreateFlags _flags = 0u) {
    ...
    namespace VKFWCommandBufferInheritance {
        struct InfoBaseHandler {
        // Data:
        ...
        // Constructors:
            ...
            InfoBaseHandler() {}
            InfoBaseHandler(const VkCommandBufferInheritanceInfo &_inheritance_info) : inheritance_info{_inheritance_info} {}
            InfoBaseHandler(const InfoBaseHandler &_handler) : inheritance_info{_handler.inheritance_info} {}
            InfoBaseHandler(InfoBaseHandler &&_handler) : inheritance_info{_handler.inheritance_info} {
                _handler.clearHandler();
            }
            virtual ~InfoBaseHandler() {
                clearHandler();
        ...
        using InfoHandler = InfoBaseHandler;
        ...
        // Constructors:
            ...
            Builder(const VKFWRenderPass::BaseHandler &_render_pass = {},
                    const VKFWFramebuffer::BaseHandler &_framebuffer = {})
                            : render_pass{_render_pass.render_pass}, framebuffer{_framebuffer.framebuffer} {}
            ~Builder() {
                clearBuilder();
            }
    ...
    namespace VKFWCommandBuffer {
        ...
        // Constructors:
            ...
            Builder(const VKFWDevice::BaseHandler &_device = {}, const VKFWCommandPool::BaseHandler &_command_pool = {})
                            : device{_device.device}, command_pool{_command_pool.command_pool} {}
            ~Builder() {
                clearBuilder();
            }
        ...
        // Create handlers/AllocateInfo struct:

            BaseHandler makeBaseHandler(void *_p_next = nullptr) {
                return { make(_p_next) };
            }

            Handler makeHandler(void *_p_next = nullptr) {
                return { make(_p_next) };
            }

            VkCommandBuffer make(void *_p_next = nullptr) {
                VkCommandBufferAllocateInfo __info = makeInfoStruct(_p_next);
                VkCommandBuffer __command_buffer = VK_NULL_HANDLE;
                if (vkAllocateCommandBuffers(device, &__info, &__command_buffer) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Command Buffer :: make", "failed to allocate command buffer"));

                return __command_buffer;
            }

            VkCommandBufferAllocateInfo makeInfoStruct(void *_p_next = nullptr) {
        ...
        // Setters:
            ...
            void setCommandPool(const VKFWCommandPool::BaseHandler &_command_pool) {
                command_pool = _command_pool.command_pool;
            }
    ...
    namespace VKFWSubmitInfo {
        ...
        // Constructors:
            ...
            InfoBaseHandler() {}
            InfoBaseHandler(const VkSubmitInfo &_submit_info) : submit_info{_submit_info} {}
            InfoBaseHandler(const InfoBaseHandler &_handler) : submit_info{_handler.submit_info} {}
            InfoBaseHandler(InfoBaseHandler &&_handler) : submit_info{_handler.submit_info} {
                _handler.clearHandler();
            }
            virtual ~InfoBaseHandler() {
                clearHandler();
        ...
        // Constructors:
            ...
            Builder(const VKFWCommandBuffer::BaseVecHandler &_command_buffers = {},
                    const VKFWSemaphore::BaseVecHandler &_wait_semaphores = {},
                    const VKFWSemaphore::BaseVecHandler &_signal_semaphores = {})
                            : command_buffers{_command_buffers.command_buffers},
                              wait_semaphores{_wait_semaphores.semaphores},
                              signal_semaphores{_signal_semaphores.semaphores} {}
            ~Builder() {
                clearBuilder();
            }
        ...
        // Setters:
            ...
            void setCommandBuffers(const VKFWCommandBuffer::BaseVecHandler &_command_buffers) {
                command_buffers = _command_buffers.command_buffers;
            }

            void setWaitSemaphores(const VKFWSemaphore::BaseVecHandler &_wait_semaphores) {
                wait_semaphores = _wait_semaphores.semaphores;
            }
            ...
            void setSignalSemaphores(const VKFWSemaphore::BaseVecHandler &_signal_semaphores) {
                signal_semaphores = _signal_semaphores.semaphores;
            }
        }; // Builder END
    }; // VKFWSubmitInfo END
    ...
    namespace VKFWPresentInfoKHR {
        ...
        // Constructors:
            ...
            InfoBaseHandler() {}
            InfoBaseHandler(const VkPresentInfoKHR &_present_info) : present_info{_present_info} {}
            InfoBaseHandler(const InfoBaseHandler &_handler) : present_info{_handler.present_info} {}
            InfoBaseHandler(InfoBaseHandler &&_handler) : present_info{_handler.present_info} {
                _handler.clearHandler();
            }
            virtual ~InfoBaseHandler() {
                clearHandler();
        ...
        // Constructors:
            ...
            Builder(const VKFWSemaphore::BaseVecHandler &_wait_semaphores = {},
                    const VKFWSwapchainKHR::BaseVecHandler &_swapchains = {})
                            : wait_semaphores{_wait_semaphores.semaphores},
                              swapchains{_swapchains.swapchains} {}
            ~Builder() {
                clearBuilder();
            }
        ...
        // Create handlers/CreateInfo struct:
            ...
            VkPresentInfoKHR makeInfoStruct(void *_p_next = nullptr) {
                ...
                __info.pResults = nullptr; // TODO: Manage results here too (it is managed in VKFWFunc)
        ...
        // Setters:
            ...
            void setSwapchains(const VKFWSwapchainKHR::BaseVecHandler &_swapchains) {
                swapchains = _swapchains.swapchains;
            }
...
//  ============  Vulkan syncronization  ============  \\
    ...
    namespace VKFWSemaphore {
        ...
        // Create handlers/CreateInfo struct:
            ...
            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkSemaphoreCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkSemaphoreCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkSemaphore make(void *_p_next = nullptr, VkSemaphoreCreateFlags _flags = 0u,
                        const VkAllocationCallbacks * _create_cb = nullptr) {
                VkSemaphoreCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkSemaphore __semaphore = VK_NULL_HANDLE;
                if (vkCreateSemaphore(device, &__info, _create_cb, &__semaphore) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Semaphore :: make", "failed to create semaphore"));

                return __semaphore;
            }
            
            VkSemaphoreCreateInfo makeInfoStruct(void *_p_next = nullptr, VkSemaphoreCreateFlags _flags = 0u) {
    ...
    namespace VKFWFence {
        ...
        // Create handlers/CreateInfo struct:
            ...
            BaseHandler makeBaseHandler(void *_p_next = nullptr, VkFenceCreateFlags _flags = 0u,
                                        const VkAllocationCallbacks * _create_cb = nullptr,
                                        const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            Handler makeHandler(void *_p_next = nullptr, VkFenceCreateFlags _flags = 0u,
                                const VkAllocationCallbacks * _create_cb = nullptr,
                                const VkAllocationCallbacks *_destroy_cb = nullptr) {
                return { make(_p_next, _flags, _create_cb), device, _destroy_cb };
            }

            VkFence make(void *_p_next = nullptr, VkFenceCreateFlags _flags = 0u,
                            const VkAllocationCallbacks * _create_cb = nullptr) {
                VkFenceCreateInfo __info = makeInfoStruct(_p_next, _flags);
                VkFence __fence = VK_NULL_HANDLE;
                if (vkCreateFence(device, &__info, _create_cb, &__fence) != VK_SUCCESS)
                    throw std::runtime_error(SVKFW_WRAPERR("VKFW Fence :: make", "failed to create fence"));

                return __fence;
            }
            
            VkFenceCreateInfo makeInfoStruct(void *_p_next = nullptr, VkFenceCreateFlags _flags = 0u) {
...
//  =================================================  \\
    ...
    namespace VKFWFunc {
        // Caller: like Builder, but the core function (vkQueueSubmit, analogous to vkCreate*) doesn't return a Vk object
        //         it essentially collects data for a function call. Deleted.

        void BeginCommandBuffer(const VKFWCommandBuffer::BaseHandler &_command_buffer,
            ...
            if (vkBeginCommandBuffer(_command_buffer.command_buffer, &__begin_info) != VK_SUCCESS)
                throw std::runtime_error(SVKFW_WRAPERR("VKFW Begin Command Buffer", "failed to begin recording command buffer"));
        }

        void EndCommandBuffer(const VKFWCommandBuffer::BaseHandler &_command_buffer) {
            vkEndCommandBuffer(_command_buffer.command_buffer);
        }

        void ResetCommandBuffer(const VKFWCommandBuffer::BaseHandler &_command_buffer, VkCommandBufferResetFlags _flags = 0u) {
            vkResetCommandBuffer(_command_buffer.command_buffer, _flags);
        }

        void QueueSubmit(const VKFWQueue::BaseHandler &_queue, const std::vector<VkSubmitInfo> &_submits,
                         const VKFWFence::BaseHandler &_fence = {}) {
            if (vkQueueSubmit(_queue.queue, _submits.size(), _submits.data(), _fence.fence) != VK_SUCCESS) {
                throw std::runtime_error(SVKFW_WRAPERR("VKFW Queue Submit", "failed to submit queue"));
            }
        }

        void QueuePresentKHR(const VKFWQueue::BaseHandler &_queue,
                             const VKFWPresentInfoKHR::InfoBaseHandler &_pres_info,
            ...
            if (vkQueuePresentKHR(_queue.queue, _pres_info.ptr()) != VK_SUCCESS) {
                throw std::runtime_error(SVKFW_WRAPERR("VKFWFunc :: QueuePresentKHR", "failed to present queue"));
            }
            else if (_check_results) {
                for (uint32_t i = 0u; i < __info.swapchainCount; ++i) {
                    if (__info.pResults[i] != VK_SUCCESS)
                        fprintf(outswarn, "%s %d\n", SVKFW_WRAPWARN("VKFWFunc :: QueuePresentKHR", "error with swapchain"), i);
                }
            }
        }

        void WaitForFences(const VKFWDevice::BaseHandler &_device, const VKFWFence::BaseVecHandler &_fences,
                           VkBool32 _wait_all = VK_TRUE, uint64_t _wait_time = UINT64_MAX) {
            vkWaitForFences(_device.device, _fences.count(), _fences.ptr(), _wait_all, _wait_time);
        }

        void ResetFences(const VKFWDevice::BaseHandler &_device, const VKFWFence::BaseVecHandler &_fences) {
            vkResetFences(_device.device, _fences.count(), _fences.ptr());
        }

        uint32_t AcquireNextImageKHR(const VKFWDevice::BaseHandler &_device, const VKFWSwapchainKHR::BaseHandler &_swapchain,
                                     const VKFWSemaphore::BaseHandler &_semaphore, const VKFWFence::BaseHandler &_fence = {},
                                     uint64_t _timeout = UINT64_MAX) {
            uint32_t __image_index = 0u;
            vkAcquireNextImageKHR(_device.device, _swapchain.swapchain, _timeout, _semaphore.semaphore, _fence.fence, &__image_index);

            return __image_index;
        }

        VKFWImage::BaseVecHandler GetSwapchainImages(const VKFWSwapchainKHR::BaseHandler &_swapchain_handle) {
            std::vector<VkImage> __images;
            uint32_t __img_count = 0u;
            vkGetSwapchainImagesKHR(_swapchain_handle.device, _swapchain_handle.swapchain, &__img_count, nullptr);
            __images.resize(__img_count);
            vkGetSwapchainImagesKHR(_swapchain_handle.device, _swapchain_handle.swapchain, &__img_count, __images.data());
            return { __images, _swapchain_handle.device };
        }
    }; // VKFWFunc END
...
//  ================================================  \\
    ...
    namespace VKFWCmd {
        ...
        // Constructors:
            ...
            BeginRenderPass(const VKFWCommandBuffer::BaseHandler &_command_buffer = {},
                            const VKFWRenderPass::BaseHandler &_render_pass = {},
                            const VKFWFramebuffer::BaseHandler &_framebuffer = {})
                            : command_buffer{_command_buffer.command_buffer}, render_pass{_render_pass.render_pass},
        ...
        // Setters:
            ...
            void setCommandBuffer(const VKFWCommandBuffer::BaseHandler &_command_buffer) {
                command_buffer = _command_buffer.command_buffer;
            }
            ...
            void setFramebuffer(const VKFWFramebuffer::BaseHandler &_framebuffer) {
                framebuffer = _framebuffer.framebuffer;
            }
            ...
            void setRenderArea(const VKFWRect2D::SctBasic &_area) {
        ...
        // Additional methods:
            ...
            void addBlackColor() {
                clear_colors.push_back(VKFWClearValue::build(0.f, 0.f, 0.f, 1.f));
            }
        }; // BeginRenderPass END
        ...
        // Constructors:
            ...
            BindPipeline(const VKFWCommandBuffer::BaseHandler &_command_buffer = {},
                         const VKFWPipeline::BaseHandler &_pipeline = {})
                            : command_buffer{_command_buffer.command_buffer}, pipeline{_pipeline.pipeline} {}
            ~BindPipeline() {
                clearCmdWrap();
            }
        ...
        // Execute command:
            ...
            static void CmdCall(const VKFWCommandBuffer::BaseHandler &_command_buffer,
                                VkPipelineBindPoint _bind_point,
                                const VKFWPipeline::BaseHandler &_pipeline) {
                vkCmdBindPipeline(_command_buffer.command_buffer, _bind_point, _pipeline.pipeline);
            }
        ...
        // Setters:
            ...
            void setPipeline(const VKFWPipeline::BaseHandler &_pipeline) {
        ...
        struct SetViewport {
            ...
            static void CmdCall(const VKFWCommandBuffer::BaseHandler &_command_buffer,
                ...
                vkCmdSetViewport(_command_buffer.command_buffer, _offset, _count, _viewports.data());
            }
        }; // SetViewport END
        ...
        struct SetScissor {
            ...
            static void CmdCall(const VKFWCommandBuffer::BaseHandler &_command_buffer,
                ...
                vkCmdSetScissor(_command_buffer.command_buffer, _offset, _count, _scissors.data());
            }
        }; // SetScissors END
        ...
        struct Draw {
            static void CmdCall(const VKFWCommandBuffer::BaseHandler &_command_buffer,
                ...
                vkCmdDraw(_command_buffer.command_buffer, _vert_count, _inst_count, _vert_offset, _inst_offset);
            }
        }; // Draw END
        ...
        struct EndRenderPass {
            static void CmdCall(const VKFWCommandBuffer::BaseHandler &_command_buffer) {
                vkCmdEndRenderPass(_command_buffer.command_buffer);
            }
        }; // EndRenderPass END
    }; // VKFWCmd END
